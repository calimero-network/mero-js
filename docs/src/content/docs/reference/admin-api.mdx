...
| `readinessCheck(): Promise<ReadinessStatus>` | `GET /ready` (throws on `503` while the node is not ready) |
...
| `queryContext(contextId, request: QueryContextRequest): Promise<QueryContextResponseData>` | `POST /contexts/{contextId}/query` (read-only methods; needs an account session from `login`) |
...
| `listMemberDevices(groupId, options?): Promise<ListMemberDevicesResponseData>` | `GET /groups/{groupId}/member-devices` |
| `sealToAccount(groupId, account, request: SealToAccountRequest): Promise<SealedEnvelope>` | `POST /groups/{groupId}/accounts/{account}/seal` |
...
| `setTeeAuthoringPolicy(groupId, request): Promise<void>` | `PUT /groups/{groupId}/settings/tee-authoring-policy` |
| `disableTeeAuthoringPolicy(groupId): Promise<void>` | `DELETE /groups/{groupId}/settings/tee-authoring-policy` |
