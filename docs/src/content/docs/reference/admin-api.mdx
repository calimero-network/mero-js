...
| `deleteContext(contextId): Promise<DeleteContextResponseData>` | `DELETE /contexts/{contextId}` |
...
| `leaveContext(contextId): Promise<void>` | `POST /contexts/{contextId}/leave` |
...
| `createDeviceAlias(request): Promise<CreateAliasResponseData>` | `POST /alias/create/device` |
| `lookupDeviceAlias(name): Promise<LookupAliasResponseData>` | `POST /alias/lookup/device/{name}` |
| `deleteDeviceAlias(name): Promise<DeleteAliasResponseData>` | `POST /alias/delete/device/{name}` |
| `listDeviceAliases(): Promise<ListAliasesResponseData>` | `GET /alias/list/device` |
...
| `deleteNamespace(namespaceId): Promise<DeleteNamespaceResponseData>` | `DELETE /namespaces/{namespaceId}` |
...
| `leaveNamespace(namespaceId): Promise<void>` | `POST /namespaces/{namespaceId}/leave` |
...
A device belongs to an account rather than to a namespace, so these read and
change the account this node speaks for. `revokeAccountDevice` takes a namespace
only to say where to publish from — it withdraws the device everywhere and
reports each namespace back. `relinkAccountDevice` only ever adds applications;
`rescopeAccountDevice` replaces the scope outright, which is how it is narrowed.

`listAccountDevices` is the listing a settings UI renders: it reports every
device the account has, revoked ones included and marked `revoked: true`, with
the applications each may speak for — so an active device and a withdrawn one
are told apart without a second call. Do not filter revoked devices out by
default; the node deliberately reports them rather than hiding them.

Revocation is **forward-only**. It withdraws a device's authority from here on
and does not retract past authorship — ops the device already signed keep
verifying and stay in the DAG. No UI built on `revokeAccountDevice` should imply
otherwise. A per-namespace `keyRotated: false` in the answer is a further
narrowing still: the device stopped writing there but can keep reading until an
admin rotates.

Every account also has one hidden account namespace.
The holder's `getNodeIdentity().accountNamespaceId` names it.
A pairing invite should carry it, so `initAccountPairing({ accountRootPublicKey, accountNamespace, namespaces: [] })` makes the device follow the account's namespaces on its own.
`listNamespaces` never lists it.

| Method | Endpoint |
| --- | --- |
| `initAccountPairing(request): Promise<AccountPairInitResponseData>` | `POST /account/pair-init` |
| `completeAccountPairing(request): Promise<AccountPairCompleteResponseData>` | `POST /account/pair-complete` |
| `relinkAccountDevice(deviceId, request?): Promise<RelinkDeviceResponseData>` | `POST /account/devices/{deviceId}/relink` |
| `rescopeAccountDevice(deviceId, request): Promise<RescopeDeviceResponseData>` | `PUT /account/devices/{deviceId}/scope` |
| `labelAccountDevice(deviceId, request): Promise<LabelDeviceResponseData>` | `PUT /account/devices/{deviceId}/label` |
| `listAccountDevices(): Promise<AccountDeviceEntry[]>` | `GET /account/devices` |
| `listAccountApplications(): Promise<AccountApplicationEntry[]>` | `GET /account/applications` |
| `revokeAccountDevice(namespaceId, request): Promise<RevokeAccountDeviceResponseData>` | `POST /namespaces/{namespaceId}/account/revoke` |
...
| Method | Endpoint |
| --- | --- |
| `createGroup(request: CreateGroupRequest): Promise<CreateGroupResponseData>` | `POST /groups` |
| `getGroupInfo(groupId): Promise<GroupInfoResponseData>` | `GET /groups/{groupId}` |
| `deleteGroup(groupId): Promise<DeleteGroupResponseData>` | `DELETE /groups/{groupId}` |
| `listGroupMembers(groupId): Promise<ListGroupMembersResponseData>` | `GET /groups/{groupId}/members` |
| `listGroupMemberDevices(groupId, options?): Promise<MemberDevicesEntry[]>` | `GET /groups/{groupId}/member-devices` |
...
| `setMemberAutoFollow(groupId, identity, request: SetMemberAutoFollowRequest): Promise<void>` | `PUT /groups/{groupId}/members/{identity}/auto-follow` |
| `leaveGroup(groupId): Promise<void>` | `POST /groups/{groupId}/leave` |
...
| `openToDelegatedExecution(groupId): Promise<{ changed, defaultCapabilities }>` | read-modify-write over the two above |
| `grantAuthorship(groupId, account): Promise<{ changed, capabilities }>` | read-modify-write over the two member-capability routes |
...
| `detachContextFromGroup(groupId, contextId): Promise<void>` | `POST /groups/{groupId}/contexts/{contextId}/remove` |
| `issueOwnershipProof(groupId, request: IssueOwnershipProofRequest): Promise<IssueOwnershipProofResponseData>` | `POST /groups/{groupId}/issue-ownership-proof` |
| `issueNamespaceOwnershipProof(groupId, request: IssueNamespaceOwnershipProofRequest): Promise<IssueOwnershipProofResponseData>` | `POST /groups/{groupId}/issue-namespace-ownership-proof` |

### Sync, signing, upgrades & migration

| Method | Endpoint |
| --- | --- |
| `syncGroup(groupId): Promise<SyncGroupResponseData>` | `POST /groups/{groupId}/sync` |
...
| `retryGroupUpgrade(groupId): Promise<RetryGroupUpgradeResponseData>` | `POST /groups/{groupId}/upgrade/retry` |
...
| `abortMigration(namespaceId): Promise<unknown>` | `POST /groups/{namespaceId}/migration/abort` |
