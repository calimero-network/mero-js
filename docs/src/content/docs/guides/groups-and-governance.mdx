...
### TEE authoring policy

Admission decides which TEEs may join; the authoring policy decides which of the
admitted ones may write as the namespace's TEE authority (`TeeOnly` storage and
`#[app::tee]` methods). It is namespace-scoped, so set it on the namespace root.

```typescript
await sdk.admin.setTeeAuthoringPolicy(namespaceId, { allowedMrtd: ['<mrtd-hex>'] });

// Turn TEE authorship off. Admitted TEEs stay members; none can author.
await sdk.admin.disableTeeAuthoringPolicy(namespaceId);
// Equivalent: an empty allowlist applies the same op.
await sdk.admin.setTeeAuthoringPolicy(namespaceId, { allowedMrtd: [] });
```

The node has no read-back route for this policy.
