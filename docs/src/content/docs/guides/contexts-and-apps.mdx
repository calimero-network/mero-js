...
<SeqDiagram
  ...
  steps={[
    { from: 0, to: 1, label: 'installApplication({ url, hash })' },
...
From a **registry** package — the SDK resolves the manifest, derives the bundle
...
During local development, install straight from a path with
